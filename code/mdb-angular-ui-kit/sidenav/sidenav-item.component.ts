...
import { MdbSidenavLayoutComponent } from './sidenav-layout.component';
...
export class MdbSidenavItemComponent implements AfterViewInit, AfterContentInit, OnDestroy {
  ...
  @HostBinding('attr.role') attributeRole = 'listitem';
  ...
  constructor(
    ...
    @Optional() @SkipSelf() private _sidenavMenu: MdbSidenavMenuDirective
  ) {}
  ...
  ngAfterViewInit(): void {
    ...
    pressLinkEvents$.subscribe(() => {
      if (this._sidenav.accordion) {
        this._sidenav.closeOtherCollapseItems(this.collapse, this._sidenavMenu);
      }

...
import { MdbSelectComponent } from '../select';
...
describe('MDB Table', () => {
  ...
  it('should select "All" option in pagination when showAllEntries input is set to true and entries input happens to equal total', () => {
    const paginationSelectComponent = fixture.debugElement
      .query(By.directive(MdbSelectComponent))
      .injector.get(MdbSelectComponent);
    expect(paginationSelectComponent.value).toBe(component.pagination.entries);

    component.pagination.showAllEntries = true;
    component.pagination.entries = component.dataSource.length;
    fixture.detectChanges();

    expect(paginationSelectComponent.value).toBe(component.pagination.allText);
  });
});

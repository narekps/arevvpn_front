  ...
  implements OnInit, OnDestroy, AfterContentInit, ControlValueAccessor
{
  ...
  get sizeClass(): string {
    return this.size === 'lg' ? 'select-lg' : this.size === 'sm' ? 'select-sm' : '';
  }
  ...
  get ariaLabelledbyValue(): string {
  ...
  private _showFilteredOptions(): void {
    ...
    this.optionGroups.toArray().forEach((group: MdbOptionGroupComponent) => {
      group.hidden = false;
    });
      ...
      if (this.filter) {
        ...
        this.selectFilter.valueChanges
          ...
          .subscribe((value: string) => {
            ...
            this.optionGroups.toArray().filter((group: MdbOptionGroupComponent) => {
              if (options.some((option: MdbOptionComponent) => option.group === group)) {
                group.hidden = false;
                return group;
              } else {
                group.hidden = true;
  ...
  handleSelectionClear(event?: MouseEvent): void {
    if (this.disabled || (event && event.button === 2)) {
      return;
    }
  ...
  private _handleOpenKeydown(event: any): void {
    ...
    } else if (((key === UP_ARROW || key === DOWN_ARROW) && event.altKey) || key === ESCAPE) {
      event.preventDefault();
      this.close();
      this._focus();

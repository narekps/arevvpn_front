...
<mdb-stepper
  ...
  [mobile]="isMobile"
  [mobileBarBreakpoint]="breakpoint"
>
  <mdb-step name="Step 1" [editable]="editable">Step 1 content</mdb-step>
  <mdb-step name="Step 2" [editable]="editable">
    <ng-template mdbStepIcon><i class="fas fa-envelope"></i></ng-template>Step 2 content</mdb-step
  >
  <mdb-step>
    <ng-template mdbStepHeader
      ><div data-testid="custom-header-name">Custom Step 3 Name</div></ng-template
    >
    Step 3 content
  </mdb-step>
</mdb-stepper>
...
describe('MDB Stepper', () => {
  describe('Basic Stepper', () => {
    ...
    it('should render custom step header if MdbStepHeaderDirective is used', () => {
      const steps: NodeListOf<HTMLLIElement> =
        fixture.nativeElement.querySelectorAll('.stepper-step');
      const stepperHeadText: HTMLSpanElement = steps[2].querySelector('.stepper-head-text');
      const customStepHeader: HTMLDivElement = stepperHeadText.querySelector(
        '[data-testid="custom-header-name"]'
      );

      expect(customStepHeader).toBeTruthy();
      expect(customStepHeader.textContent).toBe('Custom Step 3 Name');
    });

    it('should render custom step header for vertical stepper if MdbStepHeaderDirective is used', () => {
      component.orientation = 'vertical';
      fixture.detectChanges();
    ...
    it('should render custom step header icon if MdbStepIconDirective is used', () => {
      const stepperHeadIcons: NodeListOf<HTMLSpanElement> =
        fixture.nativeElement.querySelectorAll('.stepper-head-icon');
      const customStepIcon: HTMLElement = stepperHeadIcons[1].querySelector('i');

      expect(stepperHeadIcons[0].textContent).toBe('1');
      expect(customStepIcon).toBeTruthy();
      expect(customStepIcon.classList).toContain('fa-envelope');
      expect(stepperHeadIcons[1].textContent).not.toBe('2');
      expect(stepperHeadIcons[2].textContent).toBe('3');
    });

    it('should render custom step header icon for vertical stepper if MdbStepIconDirective is used', () => {
      component.orientation = 'vertical';
      fixture.detectChanges();
      const stepperHeadIcons: NodeListOf<HTMLSpanElement> =
        fixture.nativeElement.querySelectorAll('.stepper-head-icon');

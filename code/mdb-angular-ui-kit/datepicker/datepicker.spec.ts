...
describe('MDB Datepicker', () => {
  ...
  describe('Date selection', () => {
    ...
    it('should correctly parse date string with short form year', () => {
      const format = 'dd mm yy';
      const delimeters = datepicker._getDelimeters(format);
      datepicker.format = format;
      fixture.detectChanges();

      expect(datepicker._parseDate('01 02 99', format, delimeters)).toEqual(new Date(1999, 1, 1));

      expect(datepicker._parseDate('01 02 25', format, delimeters)).toEqual(new Date(2025, 1, 1));
    });
  ...
  describe('accessibility', () => {
    ...
    it('should translate day aria-label to human readable format', fakeAsync(() => {
      basicComponent.startDate = new Date(2021, 4, 15);
      basicComponent.inline = true;
      fixture.detectChanges();
      ...
      const selectedCell = document.querySelector('.datepicker-cell.selected');

      expect(datepicker._selectedDate).toEqual(new Date(2021, 4, 1));
      expect(selectedCell.getAttribute('aria-label')).toEqual('Saturday, May 1, 2021');
    }));
  });
...
describe('MDB Datepicker with form control', () => {
  let overlayContainer: OverlayContainer;
  let overlayContainerElement: HTMLElement;
  let fixture: ComponentFixture<DatepickerWithFormControl>;
  let component: DatepickerWithFormControl;
  let input: HTMLInputElement;
  let datepicker: MdbDatepickerComponent;
  ...
  it('should update form control value on date selection', fakeAsync(() => {
    component.startDate = new Date(2021, 4, 15);
    fixture.detectChanges();
    ...
    expect(component.dateValue).toEqual(new Date(2021, 4, 1));
  }));

  it('should update form control to valid date object if input value is valid', fakeAsync(() => {
    input.value = '12/12/2021';
    input.dispatchEvent(new Event('input'));
    fixture.detectChanges();
    flush();

    expect(component.dateValue).toEqual(new Date(2021, 11, 12));
  }));

  it('should set form control value to null when input value is cleared', fakeAsync(() => {
    input.value = '12/12/2021';
    input.dispatchEvent(new Event('input'));
    fixture.detectChanges();
    flush();
  ...
  it('should set form control value to null when month value in input is invalid', fakeAsync(() => {
    input.value = '12/12/2021';
    input.dispatchEvent(new Event('input'));
    fixture.detectChanges();
    flush();
  ...
  it('should set form control value to null when day value in input does not exist', fakeAsync(() => {
    input.value = '12/12/2021';
    input.dispatchEvent(new Event('input'));
    fixture.detectChanges();
    flush();
  ...
  it('should set form control value to null when day value in input is set to 0', fakeAsync(() => {
    input.value = '12/12/2021';
    input.dispatchEvent(new Event('input'));
    fixture.detectChanges();
    flush();
...
@Component({
  selector: 'mdb-datepicker-with-form-control',
  template: `
    <mdb-form-control>
      <input
        mdbInput
        [mdbDatepicker]="datepickerWithFormControl"
        [formControl]="date"
        type="text"
        class="form-control"
        id="exampleDatepicker1"
      />
      <label mdbLabel for="exampleDatepicker1" class="form-label">Select a date</label>
      <mdb-datepicker-toggle [mdbDatepicker]="datepickerWithFormControl"></mdb-datepicker-toggle>
      <mdb-datepicker
        #datepickerWithFormControl
        [startDate]="startDate"
        [confirmDateOnSelect]="confirmDateOnSelect"
      ></mdb-datepicker>
    </mdb-form-control>
  `,
  standalone: false,

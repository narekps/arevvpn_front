...
import { MdbDatepickerOptions } from './datepicker.interface';
...
export class MdbDatepickerComponent implements OnInit, AfterContentInit, OnDestroy {
  ...
  @Input()
  get options(): MdbDatepickerOptions {
    return this._options;
  }
  set options(value: MdbDatepickerOptions) {
    this._options = {
      ...this._options,
      ...value,
    };
  }
  private _options: MdbDatepickerOptions = {
  ...
  @Output() readonly viewChanged = new EventEmitter<MdbDatepickerView>();
    ...
    merge(
      ...
      fromEvent(document, 'click').pipe(
        ...
        filter((event: MouseEvent) => {
          const toggleButton = this._toggle ? this._toggle.button.nativeElement : null;
          const target = event.target as HTMLElement;
          const notToggle = toggleButton
  ...
  viewChange(): void {
    this._setView(this._currentView === 'days' ? 'years' : 'days');
  ...
  private _setView(view: MdbDatepickerView): void {
    this._currentView = view;
    this.viewChanged.emit(this._currentView);
  }
